    ...
    <form method="POST" action="/dashboard/categories/{{ $category->slug }}" class="mb-5" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('name') is-invalid
                @enderror" value="{{ old('name',$category->name) }}" required autofocus id="name" name="name">
                @error('name')
        ...
        <div class="mb-3">
            ...
            @if ($category->image)
                <img src="{{ asset('storage/' . $category->image) }}" class="d-block img-preview img-fluid mb-3 col-sm-5">
            @else
